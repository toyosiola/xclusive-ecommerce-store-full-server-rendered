...
export default function CartQuantityButton({
  ...
  return (
    <button
      ...
      className={`h-12 rounded bg-black px-6 text-2xl duration-300 hover:opacity-70 disabled:cursor-not-allowed disabled:opacity-30 ${pending ? "disabled:cursor-wait" : ""} ${decreaseBtn ? "rounded-r-none px-7" : "rounded-l-none"}`}
    >
      {value === "decrease" ? "-" : "+"}
    </button>
  );
